    ...
    if (!inLanguageCode.empty() && !pluginPath.empty()) {
      ...
      if (localizationFile.is_open()) {
        ...
        EPLJSONUtils::GetObjectByName(
          jsonData, "Localization", mLocalizationData);
      }
    }
  } catch (...) {
  }
...
std::string ESDLocalizer::GetLocalizedString(
  const std::string &inDefaultString) {
  if (sLocalizer != nullptr) {
...
std::string ESDLocalizer::GetLocalizedStringIntern(
  const std::string &inDefaultString) {
  return EPLJSONUtils::GetStringByName(
    mLocalizationData, inDefaultString, inDefaultString);
}
